    ...
    WITH
      per_alert AS (
        SELECT
          ...
          job_alert.url_id,
          (
            '{"keyword-lists": ' || json_uniq(json_agg(kw.alert_keyword)) ||
            ', "threshold": ' || COALESCE(
                            to_json(job_alert.alert_threshold),
                            'null') ||
            '}'
          )::json AS alert_attrs,
        ...
        GROUP BY job_alert.xfrm_id, xfrm_commands, xfrm_args,
                 ...
                 job_alert.alert_threshold,
                 job_alert.check_frequency_interval
      )
    SELECT
      per_alert.xfrm_id, per_alert.xfrm_commands, per_alert.xfrm_args,
      per_alert.spider_document_id, per_alert.contents_hash,
      ...
      CASE WHEN old_hash IS NULL
        ...
        ELSE json_object_agg(
                per_alert.alert_id,
                per_alert.alert_attrs
             )::jsonb
      END AS alert_ids
    FROM per_alert
    GROUP BY per_alert.xfrm_id, per_alert.xfrm_commands,
             per_alert.xfrm_args,
             per_alert.spider_document_id, per_alert.contents_hash;
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION subscribe(groupid INT, userid INT)
RETURNS void
AS $$
DECLARE
  alertid INT;
...
BEGIN
  ...
  INSERT INTO alert_x_type_cycle (type_x_cycle_id, alert_id) VALUES (4, alertid);
