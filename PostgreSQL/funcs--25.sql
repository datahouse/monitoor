...
BEGIN
  ...
  WITH
    ...
    change_insert AS (
      ...
      -- Not sure why we need this one, the backend shouldn't ever
      -- send such a thing. On the other hand, get_pending_changes_for
      -- is executed in a transaction different from this one here...
      --
      -- FIXME!
      WHERE (new_doc_id IS NOT NULL AND old_doc_id IS NOT NULL)
        OR agg_delta IS NOT NULL
