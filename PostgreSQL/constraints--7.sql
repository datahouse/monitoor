...
ALTER TABLE alert_x_url
  ADD CONSTRAINT alert_x_url_alert_id_url_id_key
...
ALTER TABLE alert_x_keyword
  ADD CONSTRAINT alert_x_keyword_alert_id_alert_keyword_id_key
    UNIQUE (alert_id, alert_keyword_id),
  ADD CONSTRAINT a_id_fk FOREIGN KEY (alert_id)
    REFERENCES alert (alert_id)
    ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT keyword_fk FOREIGN KEY (alert_keyword_id)
...
CREATE INDEX alert_user_id_idx
  ON alert(user_id);
CREATE INDEX alert_x_url_url_id_idx
  ON alert_x_url(url_id);
CREATE INDEX alert_x_url_group_url_group_id_idx
  ON alert_x_url_group(url_group_id);
CREATE INDEX spider_document_job_id
  ON spider_document(job_id);
