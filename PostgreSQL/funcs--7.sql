...
AS $$
  ...
  WITH
    reassigned_job AS (
      ...
      WHERE s.inactive AND (job_spider_uuid = s.uuid OR
                            job_spider_uuid IS NULL)
      RETURNING
        job_id AS job_id,
...
BEGIN
  ...
  IF old_hash IS NULL THEN
	...
	RETURN QUERY
      SELECT 'html2text'::TEXT AS command,
             ARRAY[]::int[] AS alert_ids;
  ELSIF old_hash != in_hash THEN
    RETURN QUERY
      SELECT x.command, array_agg(x.alert_id)
        FROM
        (
          SELECT 'html2text' AS command,
                 job_alert.alert_id
...
CREATE FUNCTION spider_store_document(in_job_id INT, in_old_doc_id INT, in_spider_uuid UUID,
                                      in_contents BYTEA, in_ts TIMESTAMP WITH TIME ZONE, alert_ids INT[])
RETURNS BIGINT
AS $$
DECLARE
  inserted_doc_id INT;
BEGIN
  INSERT INTO spider_document (job_id, contents, reception_ts)
    VALUES (in_job_id, in_contents, in_ts)
    RETURNING spider_document_id INTO inserted_doc_id;

  WITH
    info_rec AS (
      SELECT unnest(alert_ids) as alert_id
    ),
	-- lookup the last document generated for this job (needs tweaking
	-- as soon as we support different transformations, but for now,
	-- it's a relatively simply lookup by job_id)
    old_doc AS (
      SELECT MAX(spider_document_id) AS max_doc_id, job_id
      FROM spider_document
      WHERE spider_document_id <> inserted_doc_id
  ...
  SELECT i.alert_id, i.url_id, i.type_x_cycle_id,
         old_doc.max_doc_id, inserted_doc_id, in_spider_uuid, in_ts
  FROM toinsert i, old_doc
  WHERE old_doc.job_id = in_job_id;
