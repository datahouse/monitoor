...
BEGIN
  ...
  RETURN QUERY
    ...
    WHERE
      j.job_id = in_job_id AND url.url_active
   GROUP BY
    ...
    WITH
      ...
      per_check_frequency AS (
        ...
        LEFT JOIN latest_change lc
          ON lc.job_id = in_job_id
          AND lc.xfrm_id = in_xfrm_id
          AND lc.check_frequency_id = cf.check_frequency_id
        LEFT JOIN change c
          ON lc.change_id = c.change_id
        WHERE u.spider_job_id = in_job_id
          AND u.url_active
        GROUP BY
          cf.check_frequency_id, cf.check_frequency_interval, lc.xfrm_id
      )
    -- Then insert a pending change per check_frequency.
...
BEGIN
  ...
  WITH
    ...
    agg_changes AS (
      SELECT
        min(old_doc_id) AS old_doc_id,
        max(new_doc_id) AS new_doc_id,
        json_array_collapse(json_agg(delta))::jsonb AS agg_delta
    ...
    change_insert AS (
      INSERT INTO change
        (ts, old_doc_id, new_doc_id, delta,
         user_id, spider_uuid)
      SELECT in_ts, old_doc_id, new_doc_id, agg_delta,
             in_user_id, in_spider_uuid
      FROM agg_changes
  ...
  IF new_change_id IS NOT NULL THEN
    ...
    -- Also register the change in latest_change
    UPDATE latest_change
      SET change_id = new_change_id
      WHERE job_id = my_job_id
        AND xfrm_id = my_xfrm_id
        AND check_frequency_id = my_check_frequency_id;
    IF NOT found THEN
      INSERT INTO latest_change
        (job_id, xfrm_id, check_frequency_id, change_id)
      VALUES
        (my_job_id, my_xfrm_id, my_check_frequency_id, new_change_id);
    END IF;
  END IF;
