...
BEGIN
  ...
  DELETE FROM url CASCADE;
  DELETE FROM spider_change CASCADE;
  DELETE FROM spider_job CASCADE;
...
CREATE OR REPLACE FUNCTION test_url_insert_delete()
RETURNS SETOF TEXT
AS $$
DECLARE
  test_user_id INT;
...
BEGIN
  ...
  -- then delete that url
  DELETE FROM url WHERE url_id = my_url_id;
...
CREATE OR REPLACE FUNCTION test_url_insert_twice_then_delete_other()
RETURNS SETOF TEXT
AS $$
DECLARE
  test_user_id INT;
...
BEGIN
  ...
  DELETE FROM url WHERE url_id = other_url_id;
...
CREATE OR REPLACE FUNCTION test_url_insert_twice_then_delete_twice()
RETURNS SETOF TEXT
AS $$
DECLARE
  test_user_id INT;
