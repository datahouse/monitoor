...
BEGIN
  ...
  RETURN QUERY
    WITH
      x AS (
        SELECT job_alert.xfrm_id, xfrm_commands, xfrm_args,
        ...
        -- join the latest notification per alert, url and type_x_cycle
        LEFT JOIN alert_latest_notification n
          ON n.alert_id = job_alert.alert_id
          AND n.url_id = job_alert.url_id
          AND n.type_x_cycle_id = job_alert.type_x_cycle_id
  ...
  IF in_old_doc_id IS NOT NULL THEN
    WITH
      ...
      last_notification_check AS (
        SELECT
          ...
          COALESCE(
            ...
            False)
            AS is_retained,
          -- If this is the first notification to generate, we can
          -- simply use the old_doc_id provided. Otherwise, refer the
          -- latest document referenced by the last notification.
          COALESCE(a.latest_doc_id, in_old_doc_id) AS old_doc_id
        FROM info_rec
      ...
      notification_insert AS (
        ...
        SELECT alert_id, url_id, type_x_cycle_id,
               old_doc_id, inserted_doc_id, in_spider_uuid, in_ts, is_retained
        FROM last_notification_check
        RETURNING alert_id
      ),
