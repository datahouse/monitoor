...
-- ATTENTION: this file defines two dependent functions:
-- materialize_change and materialize_changes. The latter calls the
-- former.
...
BEGIN
  ...
  WITH
    ...
    change_insert AS (
      ...
      -- case, we're not creating a change at all. But still consuming
      -- the pending_change(s).
    ...
    change_url_link_insert AS (
      ...
      INNER JOIN affected_urls AS au ON au.url_id IS NOT NULL
...
ALTER FUNCTION public.materialize_change(pendingchangeids json, alertmatches json, sections json, in_user_id integer, in_spider_uuid uuid, in_ts timestamp with time zone)
  OWNER TO project_mon;
ALTER FUNCTION public.materialize_changes(changes json, in_user_id integer, in_spider_uuid uuid, in_ts timestamp with time zone)
  OWNER TO project_mon;
